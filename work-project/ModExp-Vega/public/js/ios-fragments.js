...
// Малювання гексагональної матриці із сот
class HexGridManager {
    constructor(containerOrId, serverData, options = {}) {
        ...
        if (!this.container) {
            console.error('HexGridManager: Контейнер для малювання сітки не знайдено.')
            return
        }

        // Запис вхідних JSON даних
        ...
        this.offsetX = options.offsetX || 120
        this.offsetY = options.offsetY || 110
        this.defaultFill = options.defaultFill || '#808080'
        ...
        // this.generateCoordinatesAndMapData()
        this.initGridAndMapData()
        this.drawAxes()
    ...
    initStructure() {
        ...
        // Встановлюємо віртуальну сітку координат
        // this.svg.setAttribute('viewBox', '0 0 1000 1000')

        // 1. Отримуємо реальні поточні розміри батьківського div у пікселях
        const rect = this.container.getBoundingClientRect()

        // 2. ЗАХИСТ ВІД НУЛІВ: якщо rect.width дорівнює 0, беремо клієнтську ширину або дефолтні
        this.width = /*rect.width || this.container.clientWidth ||*/ 900
        this.height = /*rect.height || this.container.clientHeight ||*/ 840

        // 3. Встановлюємо динамічний viewBox під фактичний розмір екрана
        this.svg.setAttribute('viewBox', `0 0 ${this.width} ${this.height}`)

        // 4. Розтягуємо сам SVG-тег
    ...
    // // Парсинг JSON та мапінг даних під координати сітки
    // generateCoordinatesAndMapData() {
    //     this.activeCellsData = new Map()

    //     // Перевіряємо наявність потрібних гілок у вашому JSON
    //     if (this.serverData && this.serverData.params && this.serverData.params.param) {
    //         this.serverData.params.param.forEach((item) => {
    //             // Ваші координати: nastr.i -> X, nastr.j -> Y
    //             if (item.nastr && item.nastr.i && item.nastr.j) {
    //                 const x = parseInt(item.nastr.i, 10)
    //                 const y = parseInt(item.nastr.j, 10)
    //                 const key = `${x}-${y}`

    //                 // Зберігаємо повний об'єкт соти під її унікальним ключем координат
    //                 this.activeCellsData.set(key, item)
    //             }
    //         })
    //     }

    //     // Розрахунок фізичних центрів для всього діапазону X та Y
    //     for (let y = this.startY; y <= this.endY; y++) {
    //         let rIndex = y - this.startY
    //         for (let x = this.startX; x <= this.endX; x++) {
    //             let cx = (x - this.startX) * (this.hexWidth / 2) + this.offsetX
    ...
    // 1. Генерація базової маски координат (повертає Set для швидкої перевірки)
    getBaseCoordinatesSet() {
        const rawCoords = []

        // Ваші цикли з ТЗ (крок X+2)
        for (let x = 24; x <= 34; x += 2) rawCoords.push({ x, y: 1 }, { x, y: 15 })
        for (let x = 21; x <= 37; x += 2) rawCoords.push({ x, y: 2 }, { x, y: 14 })
        for (let x = 20; x <= 38; x += 2) rawCoords.push({ x, y: 3 }, { x, y: 13 })
        for (let x = 19; x <= 39; x += 2) rawCoords.push({ x, y: 4 }, { x, y: 12 })
        for (let x = 18; x <= 40; x += 2) rawCoords.push({ x, y: 5 }, { x, y: 11 })
        for (let x = 17; x <= 41; x += 2) rawCoords.push({ x, y: 6 }, { x, y: 10 })
        for (let x = 16; x <= 42; x += 2) rawCoords.push({ x, y: 7 }, { x, y: 9 })
        for (let x = 17; x <= 41; x += 2) rawCoords.push({ x, y: 8 })

        return new Set(rawCoords.map((c) => `${c.x}-${c.y}`))
    }

    /**
     * Ініціалізація сітки та мапінгу даних
     * @param {string} mode - 'all' (показати всю базову сітку) або 'serverOnly' (тільки серверні дані)
     */
    initGridAndMapData(mode = 'serverOnly') {
        this.activeCellsSet = this.getBaseCoordinatesSet()
        this.activeCellsData = new Map()

        if (mode === 'all') {
            // Режим 1: Заповнюємо Map ВСІМА сотами з базової сітки (ТЗ)
            // Якщо сервера немає або дані пусті, вони будуть мати значення null або пустий об'єкт
            this.activeCellsSet.forEach((key) => {
                this.activeCellsData.set(key, { isPlaceholder: true })
            })
        }

        // Парсинг серверних даних (накладаємо поверх або додаємо вибірково)
        if (this.serverData?.params?.param) {
            this.serverData.params.param.forEach((item) => {
                if (item.nastr?.i && item.nastr?.j) {
                    const x = parseInt(item.nastr.i, 10)
                    const y = parseInt(item.nastr.j, 10)
                    const key = `${x}-${y}`

                    // Додаємо дані лише якщо координата в межах дозволеної базової сітки ТЗ
                    if (this.activeCellsSet.has(key)) {
                        // Перезаписуємо пусту соту реальними даними сервера
                        this.activeCellsData.set(key, { ...item, isPlaceholder: false })
                    }
                }
            })
        }

        // Єдиний розрахунок фізичних центрів для всієї сітки (без дублювання)
        for (let y = this.startY; y <= this.endY; y++) {
            const rIndex = y - this.startY
            const cy = rIndex * this.hexHeight * 0.75 + this.offsetY

            for (let x = this.startX; x <= this.endX; x++) {
                if (!this.centers[x]) this.centers[x] = {}

                this.centers[x][y] = {
                    x: (x - this.startX) * (this.hexWidth / 2) + this.offsetX,
                    y: cy,
                }
            }
        }
    ...
    drawAxes() {
        ...
        // 1. ВЕРТИКАЛЬНІ ЛІНІЇ ТА ПІДПИСИ X (ЗВЕРХУ ТА ЗНИЗУ)
        for (let x = this.startX; x <= this.endX; x++) {
            let fixedCx = this.centers[x][this.startY].x

            // Початково задаємо базові межі
            let currentTopY = lineTopY
            let currentBottomY = lineBottomY

            //
            const extraLength = 20
            const isEven = x % 2 === 0

            // Змінюємо довжину: довгі лінії виступають на 20px далі за короткі
            if (isEven) {
                currentTopY -= extraLength // Парні: лінія сильно виходить вгору
            } else {
                currentBottomY += extraLength // Непарні: line сильно виходить вниз
            }

            // Малюємо лінію
            let line = document.createElementNS('http://www.w3.org/2000/svg', 'line')
            line.setAttribute('x1', fixedCx)
            line.setAttribute('y1', currentTopY)
            line.setAttribute('x2', fixedCx)
            line.setAttribute('y2', currentBottomY)
            line.setAttribute('class', 'axis-line')
            this.axisLayer.appendChild(line)

            // Виводимо підписи та віддаляємо їх від краю ліній
            if (isEven) {
                // Підпис зверху (ще далі від краю лінії)
                let text = document.createElementNS('http://www.w3.org/2000/svg', 'text')
                text.setAttribute('x', fixedCx)
                text.setAttribute('y', currentTopY - 18)
                text.setAttribute('text-anchor', 'middle')
                text.setAttribute('class', 'axis-text')
                text.textContent = x
                this.axisLayer.appendChild(text)
            } else {
                // Підпис знизу (ще далі від краю лінії)
                let textBottom = document.createElementNS('http://www.w3.org/2000/svg', 'text')
                textBottom.setAttribute('x', fixedCx)
                textBottom.setAttribute('y', currentBottomY + 28)
                textBottom.setAttribute('text-anchor', 'middle')
                textBottom.setAttribute('class', 'axis-text')
                textBottom.textContent = x
                this.axisLayer.appendChild(textBottom)
            }
        }

        // 2. ГОРИЗОНТАЛЬНІ ЛІНІЇ ТА ПІДПИСИ Y (ЗЛІВА ТА СПРАВА)
        for (let y = this.startY; y <= this.endY; y++) {
            ...
            textRight.setAttribute('x', lineRightX + 15)
            textRight.setAttribute('y', fixedCy + 5)
            textRight.setAttribute('text-anchor', 'start')
            textRight.setAttribute('class', 'axis-text')
            textRight.textContent = `${y}`
            this.axisLayer.appendChild(textRight)
        }
    }
    ...
    getHexDistance(x1, y1, x2, y2) {
        // Переведення зміщених координат (offset) у кубічні (cube coordinates) для odd-r сітки
    ...
    getHexRingPosition(x, y, cx = 29, cy = 8) {
        ...
        const center = toCube(cx, cy)
        const target = toCube(x, y)

        const dq = target.q - center.q
        const dr = target.r - center.r
        const ds = target.s - center.s

        const radius = Math.max(Math.abs(dq), Math.abs(dr), Math.abs(ds))
        if (radius === 0) return { radius: 0, step: 0, sector: 0 }
        ...
        const dirs = [
            { q: -1, r: 1, s: 0 }, // сектор 0 -> рухаємось вниз-ліворуч
            { q: 0, r: 1, s: -1 }, // сектор 1 -> рухаємось вниз
            { q: 1, r: 0, s: -1 }, // сектор 2 -> рухаємось вгору-праворуч
            { q: 1, r: -1, s: 0 }, // сектор 3
            { q: 0, r: -1, s: 1 }, // сектор 4
            { q: -1, r: 0, s: 1 }, // сектор 5
        ]
        ...
        for (let sector = 0; sector < 6; sector++) {
            for (let j = 0; j < radius; j++) {
                if (moveQ === dq && moveR === dr && moveS === ds) {
                    return { radius, step: currentStep, sector }
                }
                moveQ += dirs[sector].q
                moveR += dirs[sector].r
                moveS += dirs[sector].s
        ...
        return { radius, step: currentStep, sector: 0 }
    }

    getHexDataByGeometry(x, y, cx = 29, cy = 8) {
        // Якщо це центр
        if (x === cx && y === cy) {
            return { radius: 0, sector: 0 }
        }

        // Беремо реальні екранні координати центрів сот
        const centerCell = this.centers[cx][cy]
        const targetCell = this.centers[x][y]

        // Вектор від центру до поточної соти
        const dx = targetCell.x - centerCell.x
        const dy = targetCell.y - centerCell.y

        // 1. Рахуємо точну відстань в сотах (радіус)
        // Відстань між центрами сусідніх сот дорівнює: h = size * sqrt(3)
        const hexHeight = this.size * Math.sqrt(3)
        const distancePixels = Math.hypot(dx, dy)

        // Округляємо до найближчого цілого радіуса
        const radius = Math.round(distancePixels / hexHeight)

        // 2. Рахуємо кут в радіанах (від -PI до PI) і переводимо в градуси (0...360)
        let angleDeg = (Math.atan2(dy, dx) * 180) / Math.PI
        if (angleDeg < 0) angleDeg += 360 // Робимо кут додатним

        // 3. Визначаємо сектор (розбиваємо 360 градусів на 6 секторів по 60 градусів)
        // Оскільки лінія йде вправо (кут 0 градусів) — це початок Сектора 0
        // Додаємо зсув 30 градусів, щоб межі секторів проходили по кутах, а не по центрах сторін
        const sector = Math.floor(((angleDeg + 30) % 360) / 60)

        return { radius, sector, angleDeg }
    }
    ...
    renderInitialGrid() {
        ...
        for (let y = this.startY; y <= this.endY; y++) {
            for (let x = this.startX; x <= this.endX; x++) {
                ...
                // Малюємо соту лише якщо її координати i та j знайдені у вашому JSON
                if (!this.activeCellsData.has(key)) continue

                const itemData = this.activeCellsData.get(key)
                const { x: cx, y: cy } = this.centers[x][y]

                // --- ЗБІР ВСІХ IDEN ДЛЯ ПОТОЧНОЇ СОТИ ---
                // Оскільки itemData — це один елемент з вашого масиву params.param
                const idenList = []

                // 1. Перевіряємо iden на верхньому рівні соти
                if (
                    itemData.iden &&
                    typeof itemData.iden === 'string' &&
                    itemData.iden.trim() !== ''
                ) {
                    idenList.push(itemData.iden.trim())
                }

                // 2. Перевіряємо iden у вкладеному під-об'єкті .param
                if (
                    itemData.param &&
                    itemData.param.iden &&
                    typeof itemData.param.iden === 'string' &&
                    itemData.param.iden.trim() !== ''
                ) {
                    idenList.push(itemData.param.iden.trim())
                }

                // Робимо список унікальним (про всяк випадок) і з'єднуємо через кому
                const uniqueIdens = [...new Set(idenList)]
                const idenAttrValue = uniqueIdens.join(',')

                const polygon = document.createElementNS('http://www.w3.org/2000/svg', 'polygon')
                const hexPointsArray = this.getHexPoints(cx, cy, this.size).split(' ')

                polygon.setAttribute('points', this.getHexPoints(cx, cy, this.size))

                polygon.setAttribute('fill', this.defaultFill)
                polygon.setAttribute('stroke', 'black')
                polygon.setAttribute('stroke-width', '1')
                polygon.setAttribute('class', 'hex')
                polygon.setAttribute('data-iden', idenAttrValue)

                this.gridLayer.appendChild(polygon)

                // // --- ДОДАВАННЯ ТОВСТОЇ ЗГАРНІ ДЛЯ 2 СТОРІН ---
                // // hexPointsArray містить 6 точок: [0] - права-верхня, [1] - нижня-права, [2], [3], [4], [5]
                // // Вибираємо 3 точки, щоб сформувати 2 суміжні сторони:

                // // ПРИКЛАД: Дві верхні сторони (кришка шестикутника): це точки [4] -> [5] -> [0]
                // const thickPoints = [hexPointsArray[1], hexPointsArray[2], hexPointsArray[3]].join(
                //     ' ',
                // )

                // const thickStroke = document.createElementNS(
                //     'http://www.w3.org/2000/svg',
                //     'polyline',
                // )
                // thickStroke.setAttribute('points', thickPoints)
                // thickStroke.setAttribute('fill', 'none') // Важливо, щоб не перекривати заливку
                // thickStroke.setAttribute('stroke', 'black') // Колір жирної лінії
                // thickStroke.setAttribute('stroke-width', '5') // Товщина жирної лінії
                // thickStroke.setAttribute('stroke-linecap', 'round') // Робить кути акуратнішими
                // thickStroke.setAttribute('style', 'pointer-events: none;') // Щоб лінія не заважала клікам на полігон

                // // this.gridLayer.appendChild(thickStroke)
                // linesToDraw.push(thickStroke)

                // // --- ЛОГІКА НА КІЛЬЦЯХ (РАДІУСАХ) ---
                // const centerX = 29
                // const centerY = 8
                // const { radius, step, sector } = this.getHexRingPosition(x, y, centerX, centerY)

                // let drawAllSides = false
                // let drawTwoSides = false

                // if (radius === 0) {
                //     // r = 0 : центральна сота
                //     drawAllSides = true
                // } else if (y === 8 && x >= centerX && x <= 41) {
                //     // Стартова лінія від (29;8) до (41;8) — завжди малюється
                //     drawTwoSides = true
                // } else if (radius > 0) {
                //     // r = 1 -> кожен елемент (step % 1 === 0)
                //     // r = 2 -> кожен 2-й елемент (step % 2 === 0)
                //     // r = 3 -> кожен 3-й елемент (step % 3 === 0) і т.д.
                //     if (step % radius === 0) {
                //         drawTwoSides = true
                //     }
                // }

                // // --- МАЛЮВАННЯ ЛІНІЙ У ВЕРХНІЙ ШАР ---
                // if (drawAllSides) {
                //     // Якщо треба всі сторони центру, малюємо закритий полігон-обведення поверх
                //     const centerStroke = document.createElementNS(
                //         'http://www.w3.org/2000/svg',
                //         'polygon',
                //     )
                //     centerStroke.setAttribute('points', hexPointsArray.join(' '))
                //     centerStroke.setAttribute('fill', 'none')
                //     centerStroke.setAttribute('stroke', 'black')
                //     centerStroke.setAttribute('stroke-width', '10')
                //     centerStroke.setAttribute('stroke-linejoin', 'round')
                //     centerStroke.setAttribute('style', 'pointer-events: none;')
                //     // this.hexLinesLayer.appendChild(centerStroke)

                //     linesToDraw.push(centerStroke)
                // } else if (drawTwoSides) {
                //     // --- ДИНАМІЧНЕ ОБЕРТАННЯ СТОРІН ЗА ГОДИННИКОВОЮ СТРІЛКОЮ ---
                //     // Базові індекси для стартової лінії (сектор 0): [1, 2, 3]
                //     // Для кожного наступного сектора додаємо +1 до індексу (циклічно через % 6)
                //     const idx1 = (1 + sector) % 6
                //     const idx2 = (2 + sector) % 6
                //     const idx3 = (3 + sector) % 6
                ...
                //     )
                //     thickStroke.setAttribute('points', thickPoints)
                //     thickStroke.setAttribute('fill', 'none')
                //     thickStroke.setAttribute('stroke', 'black')
                //     thickStroke.setAttribute('stroke-width', '10')
                //     thickStroke.setAttribute('stroke-linecap', 'round')
                //     thickStroke.setAttribute('stroke-linejoin', 'round')
                //     thickStroke.setAttribute('style', 'pointer-events: none;')

                //     // this.hexLinesLayer.appendChild(thickStroke) // Додаємо у верхній шар ліній
                //     linesToDraw.push(thickStroke)
                // }

                const centerX = 29
                const centerY = 8
                const { radius, sector, angleDeg } = this.getHexDataByGeometry(
                    x,
                    y,
                    centerX,
                    centerY,
                ...
                if (radius === 0) {
                    ...
                    // Радіус 1 — малюємо всі сусідні соти впритик
                    drawTwoSides = true
                } else if (radius > 1) {
                    // Для радіусів 2, 3 і т.д. малюємо соти, які утворюють рівні "промені" від центру.
                    // Кути головних напрямків шестикутника (за годинниковою): 0, 60, 120, 180, 240, 300 градусів.
                    // Перевіряємо, чи сота лежить приблизно на одному з цих променів:
                    const isOnRay = [0, 60, 120, 180, 240, 300].some((rayAngle) => {
                        return (
                            Math.abs(angleDeg - rayAngle) < 5 ||
                            Math.abs(angleDeg - (rayAngle + 360)) < 5
                        )
                    })
                ...
                // --- МАЛЮВАННЯ ЛІНІЙ З ПРАВИЛЬНИМ ОБЕРТАННЯМ ---
                if (drawAllSides) {
                    const centerStroke = document.createElementNS(
                        'http://www.w3.org/2000/svg',
                        'polygon',
                    ...
                    centerStroke.setAttribute('points', hexPointsArray.join(' '))
                    centerStroke.setAttribute('fill', 'none')
                    centerStroke.setAttribute('stroke', 'blue')
                    centerStroke.setAttribute('stroke-width', '10')
                    centerStroke.setAttribute('stroke-linejoin', 'round')
                    centerStroke.setAttribute('style', 'pointer-events: none;')
                    linesToDraw.push(centerStroke)
                } else if (drawTwoSides) {
                    // Автоматично вираховуємо базовий індекс на основі сектору.
                    // Сектор 0 (лінія направо) -> індекси [1, 2, 3]
                    // Якщо лінії крутяться не туди, куди треба, змініть напрямок: (1 - sector + 6) % 6
                    const idx1 = (1 + sector) % 6
                    const idx2 = (2 + sector) % 6
                    const idx3 = (3 + sector) % 6
                    ...
                    thickStroke.setAttribute('points', thickPoints)
                    thickStroke.setAttribute('fill', 'none')
                    thickStroke.setAttribute('stroke', 'blue')
                    thickStroke.setAttribute('stroke-width', '10')
                    thickStroke.setAttribute('stroke-linecap', 'round')
                    thickStroke.setAttribute('stroke-linejoin', 'round')
                    thickStroke.setAttribute('style', 'pointer-events: none;')
                ...
                // Збільшуємо розмір шрифту (наприклад, до 24 пікселів)
                iconText.setAttribute('font-size', '24')
                // Можна також зробити його жирним, якщо "X" занадто тонкий
        ...
        // Коли ВСІ полігони вже на полі — малюємо лінії поверх них одним махом
        linesToDraw.forEach((stroke) => this.gridLayer.appendChild(stroke))
    }
    ...
    // // Метод оновлення соти за її iden з інтегрованим кешуванням
    // updateCellByIden(targetIden, sensorValueText, statusValue, targetColor) {
    //     if (!targetIden) return

    //     // Пробігаємося по кешу всіх сот на карті
    //     for (const coordKey in this.cellElements) {
    //         const cell = this.cellElements[coordKey]

    //         // Перевіряємо, чи належить поточний iden сервера цій соті
    //         if (cell.idens && cell.idens.includes(targetIden)) {
    //             // --- СИСТЕМА КЕШУВАННЯ ОКРЕМОРІДНИХ СОТ ---
    //             // Перевіряємо, чи змінилися текстове значення або статус кольору від попереднього тику
    //             if (cell.lastValue === sensorValueText && cell.lastStatus === statusValue) {
    //                 // Дані не змінилися, пропускаємо оновлення цієї соти
    //                 continue
    //             }

    //             // Оновлюємо внутрішній кеш соти для наступних перевірок
    //             cell.lastValue = sensorValueText
    //             cell.lastStatus = statusValue

    //             // --- РЕНДЕРИНГ ЗМІН ---
    //             // 1. Оновлюємо текстовий надпис (значення як значення)
    //             if (sensorValueText !== undefined && sensorValueText !== null) {
    //                 cell.textLabel.textContent = sensorValueText
    //             }

    //             // 2. Фарбуємо полігон у готовий розрахований колір, отриманий від SCADA-циклу
    //             if (targetColor) {
    //                 cell.polygon.setAttribute('fill', targetColor)
    //             }
    //         }
    //     }
    // }

    updateCellByIden(targetIden, sensorValueText, targetColor, icon = null) {
        if (!targetIden) return
        for (const coordKey in this.cellElements) {
            const cell = this.cellElements[coordKey]
            // Якщо iden збігається — оновлюємо
            if (cell.idens && cell.idens.includes(targetIden)) {
                // // 1. Оновлюємо текст значення
                // if (sensorValueText !== undefined && sensorValueText !== null) {
                //     cell.textLabel.textContent = sensorValueText
                // }

                // 2. Просто зафарбовуємо соту у вхідний колір
                if (targetColor) {
                    cell.polygon.setAttribute('fill', targetColor)
                }
                ...
                // Оновлення іконки
                if (icon) {
                    cell.iconText.textContent = icon
                }
            }
        }
    }
...
class RadarZontChart {
    /**
     * @param {string|HTMLElement} container - ID елемента або сам DOM-вузол батьківського контейнера
     * @param {Object} [options] - Налаштування діаграми
     * @param {number} [options.startHour=12] - На яку "годину" спрямувати перший промінь (наприклад: 12, 3, 6, 9)
     * @param {boolean} [options.clockwise=false] - Напрямок малювання (true - за годинниковою, false - проти, як у Delphi)
     * @param {Object} [options.colors] - Кастомна палітра кольорів, якщо потрібно перевизначити дефолтні
     */
    constructor(container, options = {}) {
        ...
        // Створюємо canvas та додаємо його в контейнер
        this.canvas = document.createElement('canvas')
        this.ctx = this.canvas.getContext('2d')
        this.container.appendChild(this.canvas)

        // Налаштовуємо стилі для canvas, щоб він займав 100% батьківського блоку
        this.canvas.style.width = '100%'
        this.canvas.style.height = '100%'
        this.canvas.style.display = 'block'

        // 1. Обчислюємо зміщення базового кута на основі "годин"
        // У Canvas 0 радіан — це 3 години. Кожна година — це Math.PI / 6 радіан.
        // Повне коло — це 360(або 2*pi у радіанах, як це рахує комп'ютер).
        const targetHour = options.startHour !== undefined ? options.startHour : 12
        this.baseAngleOffset = (targetHour - 3) * (Math.PI / 6)

        // 2. Визначаємо напрямок руху по колу (користувач вирішує сам)
        this.directionSign = options.clockwise === false ? -1 : 1

        // Палітра кольорів
        this.colors = {
            backgroundSector: '#A87E7E', // caZontBrown
            outlineAndSpokes: '#000080', // Fcolors_Color2
            valuePolygon: '#A0A0A4', // caDirtyBlueZONT
            ...
            ...(options.colors || {}), // Домішуємо кастомні кольори, якщо вони передані
        }

        this.lineWidth = 2 // Базова товщина ліній
        this.data = [] // Внутрішній масив даних

        // Мапа для швидкого доступу до індексу променя за його iden (O(1) пошук)
        this.indexMap = new Map()

        // Прапорець для запобігання зайвим перемальовуванням (Debounce/Throttle через RAF)
        this.animationFrameId = null

        // Відстежуємо зміну розмірів контейнера, щоб автоматично перемальовувати
        this.resizeObserver = new ResizeObserver(() => this.triggerRender())
        this.resizeObserver.observe(this.container)
    }

    /**
     * Внутрішній метод для оновлення фізичних розмірів canvas відповідно до батьківського div
     */
    updateDimensions() {
        const rect = this.container.getBoundingClientRect()
        // Враховуємо devicePixelRatio для чіткості на Retina-дисплеях
        ...
        // // Після зміни розміру — перемальовуємо, якщо є дані
    ...
    /**
     * Допоміжний метод розрахунку координат точки на колі
     */
    ...
    /**
     * Розрахунок кута для i-го променя з урахуванням конфігурації
     */
    calculateAngle(index, totalSectors, sectorAngle) {
        // baseAngleOffset задає стартову годину (наприклад, 12)
        // directionSign регулює рух за (+1) або проти (-1) годинникової стрілки
        // До базового зміщення додаємо/віднімаємо крок сектора помножений на індекс променя
        return this.baseAngleOffset + this.directionSign * sectorAngle * index
    }

    // /**
    //  * Основний публічний метод для оновлення даних та перемалювання діаграми
    //  * @param {Array} newData - Масив об'єктів стану для кожного променя
    //  */
    // update(newData) {
    ...
    /**
     * Повне встановлення/оновлення всього масиву даних (наприклад, при ініціалізації)
     * @param {Array} newData
     */
    update(newData) {
        if (!Array.isArray(newData) || newData.length === 0) return
        ...
        // Перебудовуємо мапу індексів для швидкого пошуку
        this.indexMap.clear()
        this.data.forEach((item, index) => {
            if (item && item.iden) {
                this.indexMap.set(item.iden, index)
            }
        })
    ...
    /**
     * ТОЧКОВЕ ОНОВЛЕННЯ: Оновлює лише один промінь за його унікальним ключем 'iden'
     * @param {string} iden - Ідентифікатор параметра (наприклад, "TDR201B")
     * @param {Object} updatedFields - Об'єкт із новими значеннями (наприклад, { value: 0.7, isSsmActive: false })
     */
    updateParamById(iden, updatedFields) {
        if (!this.indexMap.has(iden)) return // Якщо такого iden немає на цьому графіку, ігноруємо

        const targetIndex = this.indexMap.get(iden)

        // Оновлюємо тільки змінені поля конкретного об'єкта, зберігаючи старі, якщо нові не прийшли
        this.data[targetIndex] = {
            ...this.data[targetIndex],
            ...updatedFields,
        }

        // Запитуємо оптимізоване перемальовування
        this.triggerRender()
    }

    /**
     * Оптимізований виклик рендерингу. (Debounce через requestAnimationFrame)
     * Запобігає ситуації, коли прилітає 100 апдейтів в одну мілісекунду, і браузер "лягає",
     * намагаючись перемалювати графік 100 разів. Графік перемалюється рівно 1 раз на квант монітора (60/120Hz).
     */
    triggerRender() {
        if (this.animationFrameId) return // Якщо перемальовування вже заплановане, нічого не робимо

        this.animationFrameId = requestAnimationFrame(() => {
            this.updateDimensions() // Оновлюємо розміри безпосередньо перед малюванням

            if (this.data.length > 0) {
                this.render()
            }

            this.animationFrameId = null // Скидаємо прапорець після успішного рендеру
        })
    }

    /**
     * Внутрішній метод рендерингу
     */
    render() {
        // Отримуємо логічні розміри (без dpr) для малювання
        const width = this.canvas.width / (window.devicePixelRatio || 1)
        const height = this.canvas.height / (window.devicePixelRatio || 1)

        // Центр колів (точка відліку)
        const centerX = Math.floor(width / 2)
        const centerY = Math.floor(height / 2)

        // Визначення базових радіусів (округлення за замовчуванням)
        ...
        // Тимчасові масиви для збереження обчислених точок та стилів
        const outerPoints = []
        const middlePoints = []
        const valuePoints = []
        ...
        // --- ЕТАП 1: Розрахунок геометрії та кольорів для кожного сектора ---
        for (let i = 0; i < totalSectors; i++) {
            // // const currentAngle = 2 * Math.PI - sectorAngle * i

            // // Math.PI / 2 зміщує початок на 12 годин (вгору)
            // // Знак мінус перед (sectorAngle * i) залишає оригінальний рух з Delphi (проти або за годинниковою)
            // const currentAngle = -Math.PI / 2 - sectorAngle * i

            const currentAngle = this.calculateAngle(i, totalSectors, sectorAngle)
            ...
            if (sectorData) {
                ...
                // Чи малювати зовнішню засічку значень
                ...
                } else if (sectorData.notDelivery === 1 || sectorData.notDelivery === 2) {
            ...
            // Динамічний радіус для графіка значень (залежить від valueFactor)
        ...
        // Повне очищення полотна
        this.ctx.clearRect(0, 0, width, height)

        // --- ЕТАП 2: Малювання шарів багатокутників ---

        // 1. Задній фон парасольки (Зовнішній контур - Великий)
        ...
        for (let i = 1; i < totalSectors; i++) this.ctx.lineTo(outerPoints[i].x, outerPoints[i].y)
        this.ctx.closePath()
        this.ctx.fill()
        this.ctx.stroke()

        // 2. Внутрішній багатокутник поточних значень (Малюється поверх фону)
        this.ctx.fillStyle = isAllDataInvalid ? this.colors.invalidData : this.colors.valuePolygon

        this.ctx.beginPath()
        this.ctx.moveTo(valuePoints[0].x, valuePoints[0].y)
        for (let i = 1; i < totalSectors; i++) this.ctx.lineTo(valuePoints[i].x, valuePoints[i].y)
        this.ctx.closePath()
        this.ctx.fill()
        this.ctx.stroke()

        // 3. Серединна пунктирна лінія (наноситься ПОВЕРХ графіка значень, щоб не перекривався)
        this.ctx.lineWidth = 4
        this.ctx.strokeStyle = this.colors.outlineAndSpokes
        this.ctx.setLineDash([2, 4]) // Емуляція (2px лінія, 4px пропуск)

        this.ctx.beginPath()
        this.ctx.moveTo(middlePoints[totalSectors - 1].x, middlePoints[totalSectors - 1].y)
        for (let i = 0; i < totalSectors; i++) this.ctx.lineTo(middlePoints[i].x, middlePoints[i].y)
        this.ctx.stroke()
        this.ctx.setLineDash([]) // Скидаємо пунктир назад у суцільну лінію

        // --- ЕТАП 3: Лінії-спиці від центра та поперечні засічки ---

        // 4. Спиці та засічки
        const outerNotchAngleWidth = sectorAngle / 10
        const innerNotchAngleWidth = sectorAngle / 3

        // Розширення зовнішнього радіусу для коректного малювання товстих засічок на краях
        const extendedOuterRadiusX = Math.round(outerRadiusX / Math.cos(outerNotchAngleWidth))
        const extendedOuterRadiusY = Math.round(outerRadiusY / Math.cos(outerNotchAngleWidth))
        ...
        for (let i = 0; i < totalSectors; i++) {
            ...
            // Зовнішня поперечна засічка значення
            ...
            // Внутрішня поперечна засічка значення (біля центра)
    ...
    /*** Метод життєвого циклу для очищення ресурсів (якщо графік видаляється з екрану)*/
    destroy() {
...
class SchemaModule {
    ...
    initListeners() {
        // Fullscreen Logic
        if (this.dom.fsBtn) {
            this.dom.fsBtn.onclick = async (event) => {
                event.preventDefault()

                try {
                    if (!document.fullscreenElement) {
                        // await document.documentElement.requestFullscreen()
                        await this.dom.mainWidget.requestFullscreen()
                    } else {
                        await document.exitFullscreen()
                    }
                } catch (err) {
                    console.warn('Повноекранний режим заблоковано політикою браузера:', err.message)
                }
            }
        }
        ...
        document.addEventListener('fullscreenchange', () => {
            ...
            const btn = this.dom.fsBtn

            if (btn) {
                // 1. Зміна тексту підказки (Title)
                btn.title = isFs ? 'Вийти з повного екрану' : 'На повний екран'

                // 2. Зміна іконки всередині кнопки
        ...
        //     // // 1. Шукаємо безпосередньо елемент соти (полігон/шлях), який має data-iden
        //     // const targetElement = event.target.closest('[data-iden]')
        //     // if (!targetElement) return

        //     // // 2. Перевіряємо, чи цей елемент знаходиться всередині нашого контейнера .schema-node
        //     // const container = targetElement.closest('.schema-node')
        //     // if (!container) return

        //     const targetElement = event.target.closest('.schema-node')
        //     if (!targetElement) return

        //     // Тепер цільовим вузлом для тултіпа є сама сота (полігон)
        ...
        // Поява тултіпа
        this.dom.nodesLayer.addEventListener('mouseover', (event) => {
            // 1. Завжди шукаємо головний контейнер (для позиціонування)
            const container = event.target.closest('.schema-node')
            if (!container) return

            let activeNode = container // За замовчуванням активним є сам контейнер (звичайний вузол)

            // 2. ПЕРЕВІРКА НА СОТУ:
            // Перевіряємо, чи мишка зараз над сотою (полигоном/шляхом всередині SVG)
            // Якщо у ваших сот є клас, наприклад .honeycomb-cell, замініть селектор на нього
            const isHexCell = event.target.matches('polygon, path, .cell')

            if (isHexCell) {
                const cellWithIden = event.target.closest('[data-iden]')
                // Якщо у цієї конкретної соти є свій iden — вона отримує найвищий пріоритет
                if (cellWithIden && container.contains(cellWithIden)) {
                    activeNode = cellWithIden
                }
            }

            // 3. Отримуємо фінальний iden з обраного елемента
            const idenStr = activeNode.dataset.iden
            if (!idenStr) return // Якщо iden немає ніде — ігноруємо

            clearTimeout(this.tooltipTimer)

            // Якщо ми вже стоїмо на цьому ж активному елементі (тій самій соті або тому ж контейнері) — виходимо
            // Якщо ми на тому самому елементі, просто оновлюємо позицію за мишкою
            if (this.state.tooltip.currentNode === activeNode) {
                this.positionGlobalTooltip(event)
                return
            }

            clearTimeout(this.activationTimer)
            this.activationTimer = setTimeout(() => {
                const idens = idenStr.split(',')
                if (idens.length === 0 || idens === '') return

                this.state.tooltip.iden = idens[0] // Беремо активний ID
                this.state.tooltip.currentNode = activeNode

                // Рендеримо контент з метаданими активного елемента (соти або контейнера)
                this.renderTooltipContent(activeNode, idens)

                this.tooltipEl.style.display = 'block'

                // Позиціонуємо ЗАВЖДИ відносно контейнера, щоб не ламати відсотки (%)
                this.positionGlobalTooltip(event)
            }, 100)
        })
        ...
        this.tooltipEl.innerHTML = `
            ...
            <div class="tooltip-content">
                ...
                <div class="tooltip-line row-name">
                    ...
                    <span class="tooltip-val name-value">${liveData?.name || meta?.info?.text || ''}</span>
                </div>
    ...
    //
    positionGlobalTooltip(event) {
        const scene = this.dom.scene
        const tooltip = this.tooltipEl
        if (!scene || !tooltip) return

        // 1. Отримуємо точні координати сцени на екрані (в пікселях)
        const sceneRect = scene.getBoundingClientRect()

        // 2. Вираховуємо позицію мишки відносно лівого верхнього кута сцени
        const mouseXInScene = event.clientX - sceneRect.left
        const mouseYInScene = event.clientY - sceneRect.top

        // 3. Переводимо координати мишки у відсотки (%) відносно сцени
        const mouseLeftPct = (mouseXInScene / sceneRect.width) * 100
        const mouseTopPct = (mouseYInScene / sceneRect.height) * 100

        // 4. Переводимо розміри тултіпа у відсотки відносно сцени для перевірки меж
        const tW = (tooltip.offsetWidth / sceneRect.width) * 100
        const tH = (tooltip.offsetHeight / sceneRect.height) * 100

        // 5. Задаємо невеликі відступи у відсотках, щоб тултіп не перекривав стрілку курсора
        const offsetX = 1.5 // зсув вправо в %
        const offsetY = 1.5 // зсув вниз в %

        let x = mouseLeftPct + offsetX
        let y = mouseTopPct + offsetY

        // 6. ПЕРЕВІРКА МЕЖ (у відсотках 0-100)

        // Права межа: якщо тултіп вилазить праворуч, перекидаємо його ліворуч від мишки
        if (x + tW > 100) {
            x = mouseLeftPct - tW - offsetX
        }

        // Нижня межа: якщо вилазить знизу, кидаємо його вище мишки
        if (y + tH > 100) {
            y = mouseTopPct - tH - offsetY
        }

        // Захист від виходу за ліву та верхню межі сцени
        if (x < 0) x = 1
        if (y < 0) y = 1

        // 7. Застосовуємо відсотки, які очікує ваша верстка
        tooltip.style.left = `${x}%`
        tooltip.style.top = `${y}%`
    }
    ...
    async renderNodes(config) {
        ...
        Object.entries(dynamic).forEach(([key, val]) => {
            ...
            items.forEach((item) => {
                ...
                // КРОК 1: Обов'язково спочатку додаємо вузол до шару DOM,
                // щоб він набув реальних розмірів (ширини/висоти), які налаштовані в CSS!
                ...
                // Algor
                node.dataset.algor = paramsObj.algor

                // Розрахунок координат у %
                const x = Math.max(0, (parseInt(item.x) / parseInt(origW)) * 100)
                const y = Math.max(0, (parseInt(item.y) / parseInt(origH)) * 100)
                ...
                if (key === 'dtext') {
                    // if (item.img.name !== 'TIMER_LARGE') node.style.width = ''
                    ...
                    const targetButtonText = String(
                        item?.img?.text?.txt || /*rawParam?.iden ||*/ '',
                    )
                    ...
                    // Отримуємо чистий масив параметрів (розгортаємо об'єкт у плаский масив)
                    const paramsObj = item.params || {}
                    const paramsArray = Object.values(paramsObj).flat().filter(Boolean)

                    // Збираємо абсолютно всі iden (і з верхнього рівня, і з вкладеного об'єкта .param)
                    const idenList = [
                        ...new Set(
                            paramsArray
                                .flatMap((p) => {
                                    const idens = []
                                    // 1. Беремо iden з верхнього рівня об'єкта (наприклад, "TDR201B")
                                    if (p.iden) idens.push(p.iden)
                                    // 2. Беремо iden з вкладеного під-об'єкта .param (наприклад, "YC00T04")
                                    if (p.param && p.param.iden) idens.push(p.param.iden)
                                    return idens
                                })
                                .filter(
                                    (iden) =>
                                        iden && typeof iden === 'string' && iden.trim() !== '',
                                ),
                        ),
                    ]
                    item._idenList = idenList

                    // Записуємо всі знайдені iden через кому в атрибут вузла
                    node.dataset.iden = idenList.join(',')
                    node._meta = item

                    // Робимо елемент node квадратним за допомогою CSS
                    // node.style.aspectRatio = '1 / 1'

                    // Оскільки SVG займає 100% висоти і ширини, очищуємо внутрішній контент і лейбли
                    node.innerHTML = ''

                    // Ініціалізуємо менеджер сітки, передаючи йому сам DOM-вузол 'node' та сирі дані 'item'
                    // Зберігаємо екземпляр прямо у DOM-елемент, щоб мати до нього доступ під час оновлення (SCADA polling)
                    const hexGridInstance = new HexGridManager(node, item)
                    node._hexGridInstance = hexGridInstance
                } else if (key === 'zont') {
                    // Отримуємо нові значення, збільшивши попередні вдвічі:
                    const targetW = w * 2
                    const targetH = h * 2

                    // Передаємо вже збільшені відсотки в стилі:
                    ...
                    // КРОК 2: Ініціалізуємо графік, коли node вже в DOM
                    const chartInstance = new RadarZontChart(node, {
                        startHour: 12,
                        clockwise: true,
                    })
                    node._radarZontChartInstance = chartInstance

                    // КРОК 3: Формуємо СТАРТОВІ дані на основі кількості знайдених iden (або параметрів),
                    // щоб графік відразу намалював початковий стан (нульовий або базовий)
                    // Кількість елементів у цьому масиві визначить кількість спиць (FXI)
                    const initialChartData = idenList.map((iden) => {
                        return {
                            iden: iden,
                            // Приводимо до шкали від -1 до 1.
                            value: 0.0, // Стартове значення (zn = 0)
                            isSsmActive: true, // SSM = true
                            notDelivery: 0, // nd = 0
                            isForbidden: 0, // zp = 0
                            emergencyType: 0, // ot = 0
                        }
                    })

                    // Якщо idenList порожній, але є paramsArray, будуємо по paramsArray
                    if (initialChartData.length === 0 && paramsArray.length > 0) {
                        paramsArray.forEach(() => {
                    ...
                    // КРОК 4: Передаємо стартові дані в екземпляр для первинного рендеру
                    if (initialChartData.length > 0) {
                        chartInstance.update(initialChartData)
                    }

                    // // Повертаємо return, щоб в кінці функції повторно не викликався appendChild(node)
                    // return
                }
    ...
    async processData(data) {
        ...
        paramArray.forEach((item) => {
            ...
            elementsSet.forEach((element) => {
                ...
                const group = groupHolder.dataset.group
                const algor = groupHolder.dataset.algor
                ...
                if (group === 'dtext') {
                    ...
                    if (groupHolder._hexGridInstance) {
                        // 1. Мапа констант кольорів палітри KART (мають бути зростаючі значення)
                        ...
                        // 2. Опис числових меж для кожного алгоритму (мають бути зростаючі значення)
                        const DELTA_TEMPERATURE_RANGES = [
                            { max: 3, color: PALETTE_COLORS.darkCharcoal },
                            { max: 12, color: PALETTE_COLORS.deepBlue },
                            { max: 15, color: PALETTE_COLORS.pureBlue },
                            { max: 21, color: PALETTE_COLORS.dodgerBlue },
                            { max: 24, color: PALETTE_COLORS.lightBlue },
                            { max: 27, color: PALETTE_COLORS.turquoise },
                            { max: 30, color: PALETTE_COLORS.sageGreen },
                            { max: 33, color: PALETTE_COLORS.oliveYellow },
                            { max: 35, color: PALETTE_COLORS.brightYellow },
                            { max: 39, color: PALETTE_COLORS.orange },
                            { max: Infinity, color: PALETTE_COLORS.coralRed },
                        ]

                        const TEMPERATURE_RANGES = [
                            { max: 70, color: PALETTE_COLORS.darkCharcoal },
                            { max: 100, color: PALETTE_COLORS.deepBlue },
                            { max: 150, color: PALETTE_COLORS.pureBlue },
                            { max: 200, color: PALETTE_COLORS.dodgerBlue },
                            { max: 260, color: PALETTE_COLORS.lightBlue },
                            { max: 278, color: PALETTE_COLORS.turquoise },
                            { max: 290, color: PALETTE_COLORS.sageGreen },
                            { max: 323, color: PALETTE_COLORS.oliveYellow },
                            { max: 327, color: PALETTE_COLORS.brightYellow },
                            { max: 350, color: PALETTE_COLORS.orange },
                            { max: Infinity, color: PALETTE_COLORS.coralRed },
                        ]

                        // Допоміжна функція пошуку кольору за лімітом
                        function findColorByLimit(ranges, numericValue) {
                            // .find() повертає перший елемент, який задовольняє умову
                            const matchedRange = ranges.find((range) => numericValue < range.max)
                            return matchedRange ? matchedRange.color : PALETTE_COLORS.defaultGray
                        }

                        function determineBrushColor(paramValue, algorithmType) {
                            // 2. Безпечне перетворення у число (на випадок якщо прийшов рядок "25" або "abc")
                            const numericValue = Number(paramValue)

                            // Перевіряємо чи перетворення успішне (якщо текст "abc", то буде NaN)
                            if (!isNaN(numericValue)) {
                                const normalizedAlgorithm = (algorithmType || '').toUpperCase()

                                if (normalizedAlgorithm === 'DT') {
                                    return findColorByLimit(DELTA_TEMPERATURE_RANGES, numericValue)
                                }

                                if (normalizedAlgorithm === 'T') {
                                    return findColorByLimit(TEMPERATURE_RANGES, numericValue)
                                }
                            }

                            // Дефолтний колір, якщо умови не пройдено
                            return PALETTE_COLORS.defaultGray
                        }

                        // 1. Нормалізація вхідних даних
                        const numericStatusValue = Number(item.status) // Колір залежить від значення першого датчика порівння з правою крайнюю гістограмою, аде якщо статус ок
                        const sanitizedStringValue = String(item.value || '').replace(',', '.') // Значення як значення

                        //
                        const targetIden = item.iden
                        const container = groupHolder.querySelector(`[data-iden*="${targetIden}"]`)
                        let calculatedColor = null
                        let icon = null

                        if (numericStatusValue != 0) {
                            calculatedColor =
                                APP_CONFIG.STATUS_COLORS[numericStatusValue] ||
                                APP_CONFIG.STATUS_COLORS.default

                            icon = 'X'
                        } else {
                            if (container) {
                                // 2. Отримуємо значення атрибуту та перетворюємо його на масив
                                const idenAttr = container.getAttribute('data-iden')
                                const idenArray = idenAttr.split(',')

                                // 3. Перевіряємо, чи є наш iden першим елементом у масиві
                                if (idenArray[0] === targetIden) {
                                    // 2. Розрахунок кольору на основі числового значення і в які межі попадає
                                    calculatedColor = determineBrushColor(
                        ...
                        // 3. Передаємо в карту: iden тегу, текстове значення, колір та сирий статус (для кешування)
                        groupHolder._hexGridInstance.updateCellByIden(
                            targetIden,
                            sanitizedStringValue,
                            calculatedColor,
                            icon,
                    ...
                    const elem = groupHolder._radarZontChartInstance

                    // Генерує ціле число від 1 до 100
                    const randomVal = Math.floor(Math.random() * 1) + 1

                    let sanitaizeValue = Number(String(item.value || '').replace(',', '.'))

                    if (elem) {
                        elem.updateParamById(item.iden, {
                            value: sanitaizeValue,
                            // isSsmActive: tag.ssm,
                            // notDelivery: tag.nd,
                            // isForbidden: tag.zp,
                            // emergencyType: tag.ot,
                        })
                    }
                }
            })
    ...
    async syncText(domNode, incomingValue, statusKey = null) {
        ...
        const name = metadata?.img?.name

        if (algor === 'DTEXT_TIMER') {
            formattedValue = this.formatTime(formattedValue)

            if (name === 'TIMER_LARGE') {
                targetTextElement.style.fontSize = '30cqmin'
            }
