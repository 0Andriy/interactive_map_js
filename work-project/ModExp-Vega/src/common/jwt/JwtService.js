...
export class JwtService {
    ...
    async sign(payload, overrideOptions = {}, context = {}) {
        ...
        const keyData = await this.keyResolver(
            { ...context, alg: opt.algorithm, kid: opt.kid },
            payload,
            'sign',
        )
        const key = await JwtService.transformToJoseKey(keyData, opt.algorithm, 'sign')
        ...
        // 🌟 ПОКРАЩЕННЯ ДЛЯ ІДЕМПОТЕНТНОСТІ (МЕРЕЖА):
        // Якщо передано фіксований iat (число секунд), використовуємо його.
        // Інакше — рахуємо динамічно, як і раніше.
        if (typeof opt.iat === 'number') {
            jwt.setIssuedAt(opt.iat)
        } else {
        ...
        // 🌟 ПОКРАЩЕННЯ ДЛЯ ІДЕМПОТЕНТНОСТІ (МЕРЕЖА):
        // Якщо передано точну дату закінчення у секундах (exp), ставимо її напряму.
        // Інакше — використовуємо динамічний інтервал (наприклад, opt.expiresIn = '15m')
        if (typeof opt.exp === 'number') {
            jwt.setExpirationTime(opt.exp)
        } else if (opt.expiresIn) {
    ...
    async verify(token, overrideOptions = {}) {
        ...
        try {
            // 1. Декодуємо токен без перевірки підпису, щоб дізнатися, хто його прислав
            const unverifiedHeader = jose.decodeProtectedHeader(token)
            const unverifiedPayload = jose.decodeJwt(token)

            // 2. Гнучко формуємо параметри перевірки
            const verifyOptions = {
            ...
            const { payload, protectedHeader } = await jose.jwtVerify(
                ...
                async (header, payload) => {
                    const keyData = await this.keyResolver(header, payload, 'verify')
                    return await JwtService.transformToJoseKey(keyData, header.alg, 'verify')
                },
                verifyOptions,
            )
            return { payload, header: protectedHeader }
        } catch (error) {
            const extendedError = new Error(`Token verification failed: ${error.message}`)
