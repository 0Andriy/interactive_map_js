...
/**
 * Менеджер для керування декількома інстансами Oracle баз даних (Multi-Tenant архітектуру).
...
export class OracleDatabaseManager {
    ...
    // ????
    async withTransaction(dbName, callbackFn) {
        const originalDbWrapper = await this.get(dbName)

        // Запускаємо транзакцію на оригінальній обгортці
        return originalDbWrapper.withTransaction(async (txClient) => {
            // Створюємо тимчасовий "транзакційний" dbManager
            const txDbManager = {
                get: async (requestedDbName) => {
                    // Якщо сервіс просить ту саму базу — віддаємо транзакційний клієнт!
                    if (requestedDbName === dbName) {
                        return txClient
                    }
                    // Якщо іншу (cross-db) — віддаємо як звичайно
                    return this.get(requestedDbName)
                },
            }

            // Запускаємо бізнес-код сервісу, передаючи йому цей тимчасовий менеджер
            return callbackFn(txDbManager)
        })
    }
}
