...
export class OracleDatabaseService {
    ...
    async _acquireConnection(internalCtx) {
        // Перевіряємо, чи є в нашому сховищі активний контекст транзакції
        const sessionCtx = this.transactionStorage.getStore()

        if (sessionCtx && sessionCtx.connection) {
            // Якщо ми в транзакції, підміняємо internalCtx на транзакційний
            internalCtx = {
                ...internalCtx,
                connection: sessionCtx.connection,
                traceId: sessionCtx.traceId,
                startTime: sessionCtx.startTime,
            }
        }
    ...
    async _internalExecute(sql, params = {}, options = {}, internalCtx = {}) {
        ...
        const traceId = internalCtx.traceId || null
    ...
    async queryStream(sql, params = {}, options = {}, internalCtx = {}) {
        ...
        try {
            this.logger?.debug?.('Starting Stream', { sql, traceId: traceId })
     ...
     *
     * @async
     * @param {function(TransactionContext): Promise<any>} callbackFn - Асинхронна функція, runInTransaction, transaction
     * що приймає об'єкт контексту з транзакційними методами.
     * Transaction Context via Execution Context
     * --- МЕТОД UNIT OF WORK ---
     *
    ...
    async withTransaction(callbackFn) {
        ...
        // Запускаємо callbackFn ВСЕРЕДИНІ сховища стореджу
        return this.transactionStorage.run(internalCtx, async () => {
            try {
                ...
                const ctx = {
                    ...
                    conn: connection,
                    // Для прокижання контексту в слой репозиторія, методи мають його приймати ctx = {}
                ...
                // Запускємо callbackFunction з контекстом для виконання
                ...
                this.logger?.error?.('Transaction failed, attempting Rollback...', {
                    error: error.message,
                    traceId,
                })

                // Блок безпечного rollback (захищає від падіння самого Oracle)
                if (connection) {
                    try {
                ...
                throw error // Прокидаємо оригінальну помилку бізнес-логіки далі
            } finally {
                await this._releaseConnection(connection, isLocal)
            }
        })
