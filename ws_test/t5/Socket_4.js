...
export class Socket {
        ...
        return new Promise((resolve, reject) => {
          ...
          const timer = setTimeout(() => {
            if (this.ackCallbacks.has(ackId)) {
              ...
              this.logger.warn(`Acknowledgement для події "${event}" таймаутнувся після ${ms}мс`, `Socket:${this.id}`);
  ...
  get broadcast() { return new BroadcastOperator(this.nsp.adapter, this.id); }
  get volatile() { return new BroadcastOperator(this.nsp.adapter, this.id).volatile; }
  to(roomName) { return new BroadcastOperator(this.nsp.adapter, this.id).to(roomName); }
  ...
  #initListeners() {
    ...
    this.rawWs.on('message', (message) => {
      ...
      } catch (err) { this.logger.error(`Помилка: ${err.message}`, `Socket:${this.id}`); }
    });
