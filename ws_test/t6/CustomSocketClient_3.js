    ...
    async #executeWithCrossTabMutex(authFn) {
        ...
        try {
            while (true) {
                ...
                // 1. Якщо лок вільний або застарів (захист від зависання вкладки на понад 10 сек)
                if (!lockOwner || now - parseInt(lockOwner, 10) > 10000) {
                    // Захоплюємо лідерство! Записуємо мітку часу цієї вкладки
                ...
                const authResultPromise = new Promise((resolve) => {
                ...
                const freshData = await authResultPromise
                if (freshData) return freshData // Успішно отримали токен від сусідньої вкладки!

                await sleep(500) // Якщо лідер зафейлився, відпочиваємо і йдемо на нове коло
            }
        } finally {
            channel.close()
        }
    ...
    async connect() {
        ...
        if (typeof this.options.authProvider === 'function') {
            try {
                ...
                this.logger?.error?.('Не вдалося отримати токени через authProvider:', authError)
                this.isConnecting = false
