 ...
 * Керує кімнатами сокета, його життєвим циклом, логуванням та відправкою пакетів.
 */
export class Socket {
    /**
     * @param {object} nsp - Простір імен (Namespace), до якого належить сокет.
     * @param {object} clientConnection - Низькорівневе TCP або WebSocket з'єднання (напр. ws інстанс).
     * @param {string} id - Унікальний ідентифікатор сокета (socket.id).
     * @param {object} handshake - Об'єкт з метаданими підключення (handshake дані).
     * @param {string} [sessionId] - Опціональний ID відновлюваної сесії
     */
    constructor(nsp, clientConnection, id, sessionId = null) {
        /**
        ...
        // Якщо sessionId не передано (нове підключення) — генеруємо унікальний рядок
