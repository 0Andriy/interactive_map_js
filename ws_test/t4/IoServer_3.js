...
export class IoServer {
  constructor(adapterFactory, logger, options = {}) {
    ...
    this.logger = logger; // DI логера на рівні ядра сервера
  ...
  #startHeartbeatLoop() {
    ...
    this.heartbeatInterval = setInterval(() => {
      this.logger.debug(`Запуск чергової перевірки "зомбі"-клієнтів. Активних сокетів на ноді: ${this.allSockets.size}`, 'Heartbeat');
  ...
  attach(httpServer) {
    ...
    httpServer.on('upgrade', (request, socket, head) => {
      ...
      if (!nsp) {
        this.logger.warn(`Запит на Upgrade відхилено: Немає відповідного Namespace для шляху "${fullPath}"`, 'IoServer');
      ...
      this.wss.handleUpgrade(request, socket, head, (ws) => {
        ...
        // Передаємо логер фабриці сокетів
  ...
  // Повне очищення сокет-сервера
  close() {
    this.logger.info('Ініційовано закриття сокет-сервера...', 'IoServer');
    
    // Зупиняємо глобальний таймер перевірки «зомбі»-клієнтів
    if (this.heartbeatInterval) {
      clearInterval(this.heartbeatInterval);
    }

    // Закриваємо кожен неймспейс по черзі
    ...
    // Закриваємо сам сервер веб-сокетів
    if (this.wss) {
      this.wss.close(() => {
