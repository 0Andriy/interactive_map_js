import { EventBus } from './EventBus.js';

export class Namespace {
  constructor(name, adapterInstance, serverOptions) {
    this.name = name;
    this.adapter = adapterInstance;
    this.serverOptions = serverOptions; // Прокидання глобальних налаштувань
    this.middlewares = [];
    this.globalEvents = new EventBus();
    this.sockets = new Map();
  }

  use(fn) { this.middlewares.push(fn); return this; }
  on(event, callback) { this.globalEvents.on(event, callback); }

  addSocket(socket) {
    this.runMiddlewares(socket, (err) => {
      if (err) {
        socket.emit('connect_error', err.message);
        socket.rawWs.close();
        return;
      }
      this.sockets.set(socket.id, socket);
      socket.join(socket.id);
      this.globalEvents.emit('connection', socket);

      socket.on('disconnect', () => {
        this.sockets.delete(socket.id);
        this.globalEvents.emit('disconnect', socket);
      });
    });
  }

  runMiddlewares(socket, callback) {
    let index = 0;
    const next = (err) => {
      if (err) return callback(err);
      if (index >= this.middlewares.length) return callback(null);
      const middleware = this.middlewares[index++];
      try { middleware(socket, next); } catch (e) { callback(e); }
    };
    next();
  }


  // Метод для коректного очищення неймспейсу
  close() {
    this.logger.info(`Коректне закриття простору назв. Кількість сокетів: ${this.sockets.size}`, `Namespace:${this.name}`);
    
    // Сповіщаємо всіх клієнтів у цьому неймспейсі про планову зупинку
    this.emitToAll('server_shutdown', { message: 'Сервер іде на перезавантаження. Будь ласка, зачекайте.' });

    // Примусово закриваємо кожен сокет
    for (const socket of this.sockets.values()) {
      // Важливо: обнуляємо gracePeriodMs для цього закриття, щоб видалення відбулося МИТТЄВО
      this.serverOptions.gracePeriodMs = 0; 
      socket.rawWs.close(1012, 'Server shutting down'); // 1012 = Service Restart
    }

    this.sockets.clear();
  }
}
