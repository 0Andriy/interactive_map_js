...
export const authenticateToken = (requiredRoles = []) => {
    return async (req, res, next) => {
        ...
        // Формат: "Bearer <token>"
        const token =
            authHeader && authHeader.startsWith('Bearer ') ? authHeader.split(' ')[1].trim() : null
        ...
        let decodedUser
        try {
            decodedUser = await jwtManager.verify(token, 'access')
        } catch (error) {
        ...
        // 3. Add user data to the request object (можна зробити запит в базу для взятя останьої інформації про нього)
