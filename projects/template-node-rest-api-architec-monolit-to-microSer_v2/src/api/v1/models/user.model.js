...
class UserModel {
    ...
    /**
     * Отримує список усіх користувачів з підтримкою пагінації та інформацією про загальну кількість.
     * @param {string} dbName - Назва бази даних, яка повинна відповідати одному з імен, визначених у файлі конфігурації
     * @param {object} [filters={}] - Об'єкт фільтрів (наприклад, { isActive: true, role: 'admin' }).
     * @param {boolean} [includeDeleted=false] - Чи включати м'яко видалених користувачів.
     * @param {number} [limit] - Максимальна кількість результатів для пагінації.
     * @param {number} [offset=0] - Зміщення для пагінації.
     * @returns {Promise<{data: Array<object>, pagination: object}>} Об'єкт, що містить масив користувачів та метадані пагінації.
    ...
    async findAll(dbName, filters = {}, includeDeleted = false, limit, offset = 0) {
        ...
        `
            ...
            const binds = {}
            const countBinds = {} // Окремий об'єкт для прив'язок до запиту COUNT

            // Логіка додавання фільтрів до базового SQL
            if (!includeDeleted) {
                sqlBase += ' AND U.DELETED_AT IS NULL'
            }

            if (filters.isActive !== undefined) {
                sqlBase += ' AND U.IS_ACTIVE = :isActive'
                binds.isActive = filters.isActive ? 1 : 0
                countBinds.isActive = filters.isActive ? 1 : 0
            }

            if (filters.isEmailVerified !== undefined) {
                sqlBase += ' AND U.IS_EMAIL_VERIFIED = :isEmailVerified'
                binds.isEmailVerified = filters.isEmailVerified ? 1 : 0
                countBinds.isEmailVerified = filters.isEmailVerified ? 1 : 0
            }

            if (filters.username) {
                // Використовуємо LIKE для часткового збігу, як у прикладі JSON
                sqlBase += ' AND LOWER(U.USERNAME) LIKE :username'
                binds.username = `%${filters.username.toLowerCase()}%`
                countBinds.username = `%${filters.username.toLowerCase()}%`
            }

            if (filters.email) {
                // Використовуємо LIKE для часткового збігу
                sqlBase += ' AND LOWER(U.EMAIL) LIKE :email'
                binds.email = `%${filters.email.toLowerCase()}%`
                countBinds.email = `%${filters.email.toLowerCase()}%`
            }
            ...
            if (filters.search) {
                ...
                countBinds.search = `%${filters.search.toLowerCase()}%`
            }

            // Запит для отримання загальної кількості записів (з тими ж фільтрами)
            // Використовуємо COUNT(DISTINCT U.USER_ID) для коректного підрахунку у випадку LEFT JOIN
            const countSql = `SELECT COUNT(DISTINCT U.USER_ID) AS TOTAL_COUNT ${sqlBase}`
            const countResult = await oracleDbManager.execute(dbName, countSql, countBinds)
            const totalCount = countResult.rows[0].TOTAL_COUNT

            // Запит для отримання користувачів з пагінацією
            let usersSql = `
            SELECT
        ...
        `
            ...
            const usersResult = await oracleDbManager.execute(dbName, usersSql, binds)

            // Трансформація даних для відповідності бажаному формату
            const users = usersResult.rows.map((user) => {
                ...
                //     roles: rolesArray, // Перетворено на масив
                // }
            })
            ...
            const currentPage =
                limit !== undefined && limit !== null && limit > 0
                    ...
                    : 1 // Якщо ліміту немає або він 0, вважаємо першу сторінку
            const totalPages =
                limit !== undefined && limit !== null && limit > 0
                    ? Math.ceil(totalCount / limit)
                    : totalCount > 0
                    ? 1
                    : 0 // Якщо ліміту немає, але є записи, то одна сторінка

            const pagination = {
                currentPage: currentPage,
                itemsPerPage: limit || totalCount, // Якщо ліміту немає, то itemsPerPage = totalCount
                totalItems: totalCount,
                totalPages: totalPages,
                offset: offset,
                hasNextPage: offset + (limit || totalCount) < totalCount, // (limit || totalCount) - якщо limit не вказано, беремо totalCount
                hasPreviousPage: offset > 0,
            }

            // Повертаємо об'єкт з даними та інформацією про пагінацію
            return {
                users: users,
            ...
            // Перекидаємо помилку далі для обробки на вищому рівні
            throw error
        }
    }
