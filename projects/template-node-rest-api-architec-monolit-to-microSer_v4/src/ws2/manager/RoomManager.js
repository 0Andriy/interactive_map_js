...
class RoomManager {
    ...
    constructor(logger = console) {
         ...
         * - send: Fucntion - Надсилає дані через WebSocket.
         */
    ...
    joinRoom(
        ...
        const clientIdentifier = clientWebSocket.username
            ? ` (User: ${clientWebSocket.username})`
            : ` (ID: ${clientWebSocket.id || ''})`
        ...
        if (room.clients.delete(clientWebSocket)) {
            ...
            if (clientRooms) {
                ...
                if (clientRooms.size === 0) {
                    this.clientRoomMap.delete(clientWebSocket) // Якщо клієнт не входить до інших кімнат, видаляємо його з мапи
                    this.logger.debug(
     ...
     * Ітеруємо по `this.clientRoomMap` для ефективнішої розсилки та активного видалення неактивних клієнтів.
     *
     * @param {string | object | Buffer | ArrayBuffer} message - Повідомлення для надсилання. Якщо об'єкт, буде серіалізовано в JSON.
    ...
    broadcastToAllClients(message) {
        ...
        const clientsToProcess = Array.from(this.clientRoomMap.keys())
