...
class AuthService {
    ...
    async register(dbName, userData) {
        try {
            ...
            if (existingUser) {
                ...
                throw CustomError.Conflict('User with this email or username already exists.', {
    ...
    async login(dbName, username, password, ipAddress = null, userAgent = null) {
        try {
            ...
            if (!user) {
                ...
                throw CustomError.Unauthorized('Invalid credentials', { username })
            }

            if (!user.IS_ACTIVE) {
                logger.warn(`Login failed: User '${username}' is inactive.`)
                throw CustomError.Forbidden('Account is inactive. Please contact support.', {
                    username,
                })
            }
            ...
            if (user.DELETED_AT) {
                ...
                throw CustomError.Forbidden(
                    'Account is deleted. Please contact support to restore.',
                    {
                        username,
                    },
                )
    ...
    async logout(dbName, refreshToken) {
        try {
            ...
            try {
                ...
                if (!decodedToken)
                    throw CustomError.Unauthorized('Invalid or missing refresh token.')
    ...
    async refreshTokens(dbName, oldRefreshToken, ipAddress = null, userAgent = null) {
        try {
            ...
            if (!decodedToken) throw CustomError.Unauthorized('Invalid or missing refresh token.')
            ...
            if (!storedRefreshToken || storedRefreshToken.IS_REVOKED) {
                ...
                throw CustomError.Unauthorized('Invalid or revoked refresh token.')
            }
            ...
            if (!user || !user.IS_ACTIVE || user.DELETED_AT) {
                ...
                await refreshTokenModel.revoke(dbName, oldRefreshToken)
                throw CustomError.NotFound('User account not found or is inactive.', { userId })
            }
