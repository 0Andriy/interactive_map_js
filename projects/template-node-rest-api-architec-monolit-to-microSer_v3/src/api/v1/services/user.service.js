...
class UserService {
    ...
    async getUserById(dbName, userId) {
        ...
        if (!user) {
            throw CustomError.NotFound(`User with ID ${userId} not found`, { userId })
        }
    ...
    async updateUser(dbName, userId, updateData) {
        ...
        if (!ok) {
            throw CustomError.NotFound(`Cannot update user with ID ${userId}`, { userId })
        }
    ...
    async adminChangeUserPassword(dbName, userId, newPassword) {
        try {
            ...
            if (!user) {
                ...
                throw CustomError.NotFound('User not found', { userId })
            }
            ...
            if (isNewPasswordSame) {
                ...
                throw CustomError.BadRequest('New password cannot be the same as the old password')
            }
    ...
    async changeOwnPassword(dbName, userId, oldPassword, newPassword) {
        try {
            ...
            if (!isPasswordValid) {
                ...
                throw CustomError.Unauthorized('Invalid old password')
            }
