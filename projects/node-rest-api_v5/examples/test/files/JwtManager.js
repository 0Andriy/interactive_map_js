...
class JwtManager {
     ...
     * @type {Logger}
     */
    #logger = JwtManager.#defaultLogger // Use the static default logger
     ...
     */
    constructor(userConfig = {}, customLogger = null) {
     ...
     */
    static getInstance(userConfig = {}, customLogger = null) {
        return JwtManager.#instance || new JwtManager(userConfig, customLogger)
    }
    ...
    async getKey(tokenType) {
        const cfg = this.config[tokenType]
        if (!cfg) {
    ...
    async sign(payload, tokenType = 'access', options = {}) {
        ...
        if (finalExpiresIn !== null && finalExpiresIn !== undefined) {
            jwtSigner.setExpirationTime(finalExpiresIn) // Встановлюємо термін дії
        }
    ...
    async verify(token, tokenType = 'access', options = {}) {
        ...
        try {
            ...
            this.#logger.error(`Token verification failed for type '${tokenType}': ${errorMessage}`)
        ...
        if (typeof cfg.payloadValidator === 'function') {
            ...
            if (!validationResult || !validationResult.isValid) {
                ...
                this.#logger.error(`Payload validation failed for type '${tokenType}': ${errors}`)
    ...
    decode(token) {
        try {
            ...
            this.#logger.error('Помилка декодування JWT токена:', error.message)
            return null
        }
    }
    ...
    parseExpiresIn(str) {
        ...
        if (!match) {
            this.#logger.warn(`Некоректний формат expiresIn: '${str}'.`)
        ...
        if (!multipliers[unit]) {
            this.#logger.warn(`Невідома одиниця часу expiresIn: '${unit}' у '${str}'.`)
