import { WebSocketServer } from 'ws' // Для роботи з WebSocket
...
class WsAdapter {
    ...
    #logger
    #socketToNamespaceMap = new Map() // Map<socket, namespaceId>
    #socketToUserIdMap = new Map() // Map<socket, userId> - для відстеження приєднань до кімнат
     ...
     * @param {any} message - Повідомлення для відправки.
     */
    broadcastToUsers(userIds, message) {
        ...
        userIds.forEach((userId) => {
            ...
            if (targetWs && targetWs.readyState === WebSocket.OPEN) {
                ...
                this.#logger.warn(
                    `Failed to broadcast message to user '${userId}': Connection not found or not open.`,
                )
            }
        })
    }
