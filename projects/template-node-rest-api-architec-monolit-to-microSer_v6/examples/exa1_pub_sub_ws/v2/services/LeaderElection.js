...
import { IStorage } from '../interfaces/IStorage.js' // Використовуємо IStorage для абстракції
...
class LeaderElection {
     ...
     */
    async start() {
        this.#logger.log(`Instance '${this.#instanceId}' attempting to become leader...`)
    ...
    async stop() {
        ...
        if (this.#isLeader) {
            ...
            // Спробувати звільнити блокування, але це не завжди гарантовано,
            // якщо блокування має автоматичний TTL.
            // Може бути більш доцільним просто дозволити TTL завершитися.
            await this.#storage
                .delete(this.#leaderKey)
                .catch((e) =>
                    this.#logger.error(`Failed to delete leader key on stop: ${e.message}`),
                )
            this.#isLeader = false
        }
        this.#logger.log(`LeaderElection stopped for instance '${this.#instanceId}'.`)
    }
