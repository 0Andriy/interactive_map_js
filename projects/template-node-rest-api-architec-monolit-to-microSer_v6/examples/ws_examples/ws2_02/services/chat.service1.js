...
const handleChatConnection = async (ws) => {
    ...
    await roomManager.joinRoom(
        ...
        async (roomName, clients) => {
            roomManager.logger.info(`Генеруємо бінарні дані для ${roomName}...`)
            return {
                type: 'CHAT_NEW_MESSAGE',
                username: ws.username,
                userId: ws.userId,
                room: roomName, // Вказуємо кімнату, звідки надійшло повідомлення
                text: `1111111111111111`,
                timestamp: Date.now(),
            }
        },
        // async () => {
        //     roomManager.logger.info(`Генеруємо бінарні дані для ${MAIN_CHAT_ROOM}...`)
        //     ws.send(
        //         JSON.stringify({
        //             type: 'CHAT_NEW_MESSAGE',
        //             username: ws.username,
