...
class RoomsManager {
    ...
    sendMessageToRoom(roomName, message) {
        ...
        clientsToProcess.forEach((clientWebSocket) => {
            ...
            if (clientWebSocket.readyState === WebSocket.OPEN) {
                        ...
                        Buffer.isBuffer(payloadToSend) || payloadToSend instanceof ArrayBuffer
                            ...
                            : String(payloadToSend).substring(0, 50) +
                              ...
                              String(payloadToSend).length > 50
                                  ...
                                  : '')
                    this.logger.debug(
                        `Надіслано до кімнати '${roomName}': '${logMessage}' конекту: ${clientIdentifier}`,
                    )
                } catch (error) {
                    this.logger.error(
                        `Помилка надсилання повідомлення конекту: ${clientIdentifier} у кімнаті '${roomName}': ${error.message}`,
                        error,
                    )
                ...
                this.logger.warn(
                    `Конект: ${clientIdentifier} у кімнаті '${roomName}' неактивний (readyState: ${clientWebSocket.readyState}). Видалення конекту.`,
                )
    ...
    broadcastToAllClients(message) {
        ...
        for (const clientWebSocket of connectionsToProcess) {
            ...
            if (clientWebSocket.readyState === WebSocket.OPEN) {
                        ...
                        Buffer.isBuffer(payloadToSend) || payloadToSend instanceof ArrayBuffer
                            ...
                            : String(payloadToSend).substring(0, 50) +
                              ...
                              String(payloadToSend).length > 50
                                  ...
                                  : '')
                    this.logger.debug(
                        `Глобальна розсилка: '${logMessage}' конекту: ${clientIdentifier}.`,
                    )
                } catch (error) {
                    this.logger.error(
                        `Помилка глобальної розсилки конекту ${clientIdentifier}: ${error.message}`,
                        error,
                    )
                    this.removeClientGlobally(clientWebSocket.id) // Видаляємо проблемний конект
                }
                ...
                this.logger.warn(
                    `Конект ${clientIdentifier} неактивний (readyState: ${clientWebSocket.readyState}) під час глобальної розсилки. Видалення.`,
                )
                this.removeClientGlobally(clientWebSocket.id) // Видаляємо неактивний конект
            }
        }
    ...
    sendMessageToClient(connectionId, message) {
        ...
        if (targetClient.readyState === WebSocket.OPEN) {
            try {
                ...
                const logMessage =
                    Buffer.isBuffer(payloadToSend) || payloadToSend instanceof ArrayBuffer
                        ? `[Binary Data, ${targetClient.byteLength || targetClient.length} bytes]`
                        : String(payloadToSend).substring(0, 50) +
                          (typeof payloadToSend === 'string' && String(payloadToSend).length > 50
                              ? '...'
                              : '')
                this.logger.debug(
                    `Надіслано приватне повідомлення конекту '${clientIdentifier}': '${logMessage}'.`,
                )
                return true
            } catch (error) {
                this.logger.error(
                    `Помилка надсилання приватного повідомлення конекту ${clientIdentifier}: ${error.message}`,
                    error,
                )
                this.removeClientGlobally(targetClient.id) // Видаляємо проблемний конект
                return false
            }
        } else {
            this.logger.warn(
                `Конект ${clientIdentifier} неактивний (readyState: ${targetClient.readyState}). Повідомлення не надіслано.`,
            )
            this.removeClientGlobally(targetClient.id) // Видаляємо неактивний конект
            return false
        }
    }
                        ...
                        Buffer.isBuffer(payloadToSend) || payloadToSend instanceof ArrayBuffer
                            ...
                            : String(payloadToSend).substring(0, 50) +
                              ...
                              String(payloadToSend).length > 50
                                  ...
                                  : '')
                    this.logger.debug(
                        `Надіслано користувачу '${userId}' через конект ${clientIdentifier}: '${logMessage}'.`,
                    )
                } catch (error) {
                    this.logger.error(
                        `Помилка надсилання повідомлення користувачу '${userId}' через конект ${clientIdentifier}: ${error.message}`,
                        error,
                    )
                    this.removeClientGlobally(connectionId) // Видаляємо проблемний конект
                }
            } else {
                this.logger.warn(
                    `Конект ${clientIdentifier} для користувача '${userId}' неактивний (readyState: ${clientWebSocket.readyState}). Видалення.`,
                )
                this.removeClientGlobally(connectionId) // Видаляємо неактивний конект
            }
        }
        return sentCount
    }
    ...
    getUserConnections(userId) {
        return this.userConnectionsMap.get(userId)
    }
}
